...
<aside>
The `%>%` operator requires loading the `tidyverse` package (specifically `magrittr` package). Since R 4.1.0, one can use `|>` for piping without loading any packages. While in the future `|>` is probably the preferred way, for now we'll stick to `%>%` to make things compatible for older versions of R.
</aside>
